...
//		});
	...
	public void SendAct(String mail,String act_Name,String act_Loc,String act_Date,String act_Time,String act_Intro,String act_Image,String mb_Name) {
		// ---------------------------------------------------------連線設定
		Properties prop = new Properties();
...
//	});
		...
		try {
			...
			// 標題
			subject = mb_Name+"這是您的活動報名資訊:"+act_Name;
			message.setSubject(subject);

			// 內容/格式
			txt = "<h2>"+act_Name+"</h2>"
				+"<img src=\""+act_Image+"\" style=\"width: 200px; height: 200px; text-align:center; border-radius: 50%;\"/>"
				+"<h4>"+act_Date+"</h4>"
				+"<h4>"+act_Time+"</h4>"
				+"<h4>"+act_Loc+"</h4>"
				+"<h4>"+act_Intro+"</h4>";
